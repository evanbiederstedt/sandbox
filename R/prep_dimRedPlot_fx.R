...
fx.add_factors_to_redDim_df <- function(object, df_to_plot,
  ...
  if(!is.null(color_by)){
    ...
    if(is.list(color_by)){
      ABCutilities::check_columns(c("result","title"), color_by,
                                  "list given as color_by","fx.add_factors_to_redDim_df")
      df_with_cols$color_by <- color_by$result
      names(df_with_cols)[names(df_with_cols)=="color_by"] <- color_by$title
    }else{
      color_by_out <- fx.choose_vis_values_sce(object,
                                               color_by,
                                               cell_control_default = TRUE,
                                               check_features = TRUE,
  ...
  if(!is.null(shape_by) && shape_by != color_by){
    shape_by_out <- fx.choose_vis_values_sce(object,
                                             shape_by,
                                             cell_control_default = TRUE,
  ...
  if(!is.null(size_by) && !(size_by %in% c(shape_by, color_by))){
    if(is.numeric(size_by)){
      ...
      size_by_out <- fx.choose_vis_values_sce(object,
                                            size_by,
                                            check_features = TRUE,
                                            check_coldata = TRUE,
                                            exprs_values = exprs_values)
  ...
  if(!is.null(circle_by) && !(circle_by %in% c(shape_by, color_by, size_by))){
    circle_by_out <- fx.choose_vis_values_sce(object,
                                              circle_by,
                                              check_features = FALSE,
                                              check_coldata = TRUE,
                                              exprs_values = NULL,
...
fx.choose_vis_values_sce <- function(x, by, check_coldata = TRUE,
  ...
  if (is.character(by)) {
    ...
    if (is.null(vals)) {
      warning(sprintf("'%s' not found %s", by,
