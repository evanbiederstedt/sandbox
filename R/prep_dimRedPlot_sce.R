#' Prepare the "DimRedPlot" object for plotting reduced dimension information
#' 
...
#' @import scater
#' 
get_reducedDimPlot.sce <- function(object, which_reddim = NULL,
                                     which_pcs = c(1,2),
                                     X = NULL, Y = NULL,
                                  dim_red_type = NULL,
    ...
    if(is.null(which_pcs)){
      ...
      if( !is.numeric(which_pcs)){
        ...
        if( length(missing_pc) > 0){
          stop( paste0("The component(s) that you specified (",  
                       paste(missing_pc, collapse=", "),") aren't part of the reduced dimension
  ...
  if( !is.null(color_by) | !is.null(shape_by)  | !is.null(size_by) | !is.null(circle_by) ){
    df_to_plot <- fx.add_factors_to_redDim_df(object, df_to_plot,
                                              color_by, shape_by,
                                              size_by, circle_by,
                                              exprs_values)
  }
